...
def linear_data_generator(batch_size, seq_len, d_x, d_y, device='cpu', generator=None):
    """生成一批线性回归数据。
    ...
    x_data = torch.randn(batch_size, seq_len // 2, d_x, device=device, generator=generator)
    y_data = x_data @ w
    return x_data, y_data


def dataloader(batch_size=64, seq_len=80, d_x=20, d_y=1, device='cpu',
               data_type='linear', sink_padding=1, generator=None):
    """无限批量数据加载器（生成器）。
    ...
    while True:
        if data_type == 'linear':
            x_data, y_data = linear_data_generator(
                batch_size, seq_len, d_x, d_y, device=device, generator=generator,
            )
